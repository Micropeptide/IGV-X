# IGV-X v2.19.5-igvx.2 — Release Notes
...
### First-run update setup + automatic update checks
- On first launch, IGV-X asks whether you want it to check for updates from
  micropeptide's GitHub releases automatically, and how often:
  **once a day**, **once a week**, or **never** (manual only).
- When automatic checking is enabled, IGV-X quietly checks on the chosen
  schedule and only notifies you when a new version is actually available.
- The interval is configurable in Preferences > Updates (IGV-X) —
  "Automatic update check interval in hours".

### About IGV-X
- The About dialog now names the customization: "A customized version of IGV
  built specifically for micropeptide's research (Arabidopsis epigenetics /
  WGBS)" with a pointer to Help > Check for Updates.

## In-app updates (from v2.19.5-igvx.1)
