...
- `docs/` tree (build, architecture, chromosome-resolution, sessions, tests, upstream-update, release, limitations).

### Fixed (planned)
- **bigWig chromosome-resolution NPE** (upstream 2.19.5): `BBFile.getIdForChr()` returning null on TAIR10 WGBS sessions
  (bigWigs `Chr1..ChrM` vs genome tair10 canonical `NC_003070.9`/`chr1`). Missing mapping must never NPE.
